...
const UNIVERSAL_PROPS = [
	{
		...
		type: 'string',
		description:
			'Unique within the block. Used by parentPanelId references.',
	},
	{
		name: 'type',
		type: 'string',
		description: 'Control type. Editing this rebuilds the property set.',
	},
	{ name: 'label', type: 'string', description: 'Inspector label.' },
	{
		...
		type: 'string',
		description:
			'WP attribute name. Becomes attributes.<key> on the block.',
	},
	{
		name: 'default',
		description: 'Initial value matching the control type.',
	},
	{
		name: 'placeholder',
		type: 'string',
		description: 'Empty-state hint text.',
	},
	{
		name: 'helpText',
		type: 'string',
		description: 'Short description shown under the field.',
	},
	{
		name: 'parentPanelId',
		type: 'string',
		ref: 'panel',
		description: 'Nest under a group/panel/tools-panel by its id.',
	},
	{
		name: 'conditionalLogic',
		type: 'object',
		description:
			'Show only when another control matches. { field, operator, value }.',
	},
	{
		name: 'validation.required',
		type: 'boolean',
		description: 'Block save until filled in.',
	},
	{
...
function isMandatoryProp( propName, fieldType ) {
	if ( MANDATORY_PROPS.includes( propName ) ) {
		return true;
	}
	if ( propName === 'attributeKey' && ! isStructural( fieldType ) ) {
		return true;
	}
	return false;
}
...
async function getControlDocs( type ) {
	...
	if ( docsCache.has( type ) ) {
		return docsCache.get( type );
	}
	try {
		...
		return (
			<div className="gcb-builder" style={ S.page }>
				<header style={ S.header }>
					...
					<p style={ S.lede }>
						Post / taxonomy / options-page / user meta — one schema
						per file in your active theme.
					</p>
				</header>
				<StructuredFieldsApp />
			</div>
	...
	return (
		<div style={ S.page }>
			<header style={ S.header }>
				...
				<p style={ S.lede }>
					One schema per block in your active theme. Keyboard-driven
					editor: Enter / Tab / arrow keys.
				</p>
			</header>
			<BlocksApp />
		</div>
...
function StructuredFieldsApp() {
	...
	if ( view.name === 'edit' ) {
		return (
			<StructuredEditFields
				...
				onBack={ () => setView( { name: 'list' } ) }
				onPicked={ ( kind ) =>
					setView( { name: 'edit', kind, id: view.id } )
				}
			/>
		);
	}
	...
	return (
		<StructuredList
			...
			onStartNew={ ( id ) =>
				setView( { name: 'edit', kind: null, id, pendingNew: true } )
			}
		/>
	);
}
...
function StructuredList( { onEdit, onStartNew } ) {
	...
	if ( state.loading ) {
		return <p style={ S.muted }>Loading structured fields…</p>;
	}
	if ( state.error ) {
		return <Notice type="error">{ state.error }</Notice>;
	}
	...
	rows.push( {
		...
		id: 'user',
		path: d.user?.path || '',
		exists: !! d.user?.exists,
		source: d.user?.source,
	} );
	...
	const submitNew = () => {
		...
		if ( ! /^[a-z][a-z0-9_-]*$/.test( slug ) ) {
			setCreateError(
				'Slug must be lowercase letters, digits, underscores, hyphens; starting with a letter.'
			);
			return;
		}
		setNewId( '' );
	...
	return (
		<div>
			<div style={ S.toolbar }>
				...
				>
					<span style={ S.muted }>
						{ rows.filter( ( r ) => r.exists ).length } active
					</span>
					<button
						type="button"
			...
			<div style={ S.listCard }>
				{ rows.map( ( row, i ) => (
					<StructuredRow
						...
						last={
							i === rows.length - 1 &&
							! true /* always show add row below */
						}
...
const NewInlineInput = forwardRef( function NewInlineInput(
	...
	return (
		...
		>
			...
			/>
			{ creating && (
				<span style={ { ...S.muted, fontSize: 12 } }>Creating…</span>
			) }
		</div>
	);
} );
...
function StructuredEditFields( { kind, id, pendingNew, onBack, onPicked } ) {
	...
	return (
		<StructuredEditFieldsInner kind={ kind } id={ id } onBack={ onBack } />
	);
}
	...
	const options = [
		{
			kind: 'post',
			label: 'Post type',
			hint: 'CPT meta (e.g. project, testimonial)',
		},
		{
			kind: 'taxonomy',
			label: 'Taxonomy',
			hint: 'Term meta (e.g. category, post_tag)',
		},
		{
			kind: 'options',
			label: 'Options',
			hint: 'Top-level options page (e.g. site)',
		},
		{
			kind: 'user',
			label: 'User',
			hint: 'User profile fields (one schema, no slug)',
		},
	];
	return (
		<div>
			<button
				type="button"
				onClick={ onBack }
				style={ { ...S.backLink, marginBottom: 16 } }
			>
				← Field sets
			</button>
			<h2 style={ { ...S.h2, marginBottom: 6 } }>
				New field set:{ ' ' }
				<code style={ { fontFamily: T.mono, fontSize: 18 } }>
					{ id }
				</code>
			</h2>
			<p style={ { ...S.muted, marginTop: 0, marginBottom: 24 } }>
				What kind of field set is this?
			</p>
			<div
				style={ {
					display: 'grid',
...
function StructuredEditFieldsInner( { kind, id, onBack } ) {
	...
	const setDisplayWhen = ( dw ) =>
		setOtherKeys( ( prev ) => {
			const next = { ...prev };
			if ( dw == null ) {
				delete next.displayWhen;
			} else {
				next.displayWhen = dw;
			}
			return next;
	...
	useEffect( () => {
		api( endpoint ).then(
			( data ) => {
				...
				const controls = Array.isArray( content.controls )
					? content.controls
					: [];
	...
	const updateField = ( idx, f ) =>
		setFields(
			fields.map( ( existing, i ) => ( i === idx ? f : existing ) )
		);
	...
	if ( loadError ) {
		return <Notice type="error">{ loadError }</Notice>;
	}
		...
		<div>
			...
			>
				<button
					type="button"
					onClick={ onBack }
					style={ { ...S.backLink, marginBottom: 0 } }
				>
					← Field sets
				</button>
				<div
			...
			<div style={ S.editLayout }>
				<section
					style={ S.fieldList }
					aria-label="Fields in this schema"
				>
					{ fields.map( ( f, idx ) => (
						<FieldRow
							key={ idx }
				...
				<aside style={ S.propPane } aria-label="Properties">
					{ fields[ selectedIdx ] ? (
						...
						<p style={ S.muted }>
							Add a field on the left to start.
						</p>
					) }
				</aside>
			</div>
...
function BlockList( { onCreated, onEdit } ) {
	...
	function reload() {
		api( 'blocks' ).then(
			...
			( err ) =>
				setState( { loading: false, blocks: [], error: err.message } )
		);
	}

	if ( state.loading ) {
		return <p style={ S.muted }>Loading blocks…</p>;
	}
	if ( state.error ) {
		return <Notice type="error">{ state.error }</Notice>;
	}
	...
	const themeBlocks = state.blocks.filter(
		( b ) => b.target_kind !== 'plugin-examples'
	);
	const exampleBlocks = state.blocks.filter(
		( b ) => b.target_kind === 'plugin-examples'
	);
	...
	return (
		<div>
			<div style={ S.toolbar }>
				...
				>
					<span style={ S.muted }>
						{ themeBlocks.length } in active theme
					</span>
					<button
						type="button"
						onClick={ () => newInputRef.current?.focus() }
			...
			{ exampleBlocks.length > 0 && (
				<details style={ { marginTop: 32 } }>
					...
					>
						Plugin-bundled example blocks ({ exampleBlocks.length })
						— read-only
					</summary>
					<div
						style={ { ...S.listCard, marginTop: 8, opacity: 0.7 } }
					>
				...
				>
					...
					{ block.target_kind === 'draft' && (
						<span
							...
							title="In the GCB draft workspace — deploy it to take it live."
						>
							draft
						</span>
					) }
...
function EditFields( { slug, onBack } ) {
	...
	if ( loading ) {
		return <p style={ S.muted }>Loading { slug }…</p>;
	}
	if ( loadError ) {
		return <Notice type="error">{ loadError }</Notice>;
	}
	...
	return (
		<div>
			...
			<BlockMetaStrip slug={ slug } meta={ meta } onChange={ setMeta } />
			...
			<div style={ S.editLayout }>
				<section
					style={ S.fieldList }
					aria-label="Fields in this block"
				>
					{ fields.map( ( f, idx ) => (
						<FieldRow
							key={ idx }
...
function parseDisplayWhen( dw ) {
	...
	if (
		typeof dw === 'object' &&
		! Array.isArray( dw ) &&
		Array.isArray( dw.any )
	) {
		return dw.any.map( ( branch ) => {
			if ( Array.isArray( branch?.all ) ) {
				return { rules: branch.all.map( normaliseRule ) };
			}
			if ( branch && typeof branch === 'object' && branch.key ) {
				return { rules: [ normaliseRule( branch ) ] };
			}
			if ( Array.isArray( branch ) ) {
				return { rules: branch.map( normaliseRule ) };
			}
			return { rules: [] };
		} );
	}
...
function serialiseDisplayWhen( groups ) {
	...
	if ( clean.length === 1 ) {
		if ( clean[ 0 ].rules.length === 1 ) {
			return clean[ 0 ].rules[ 0 ];
		}
		return { all: clean[ 0 ].rules };
	}
	return {
		any: clean.map( ( g ) =>
			g.rules.length === 1 ? g.rules[ 0 ] : { all: g.rules }
		),
	};
}
...
function LocationRulesEditor( { kind, value, onChange } ) {
	...
	const lastSerialised = useRef(
		serialiseDisplayWhen( parseDisplayWhen( value ) )
	);
	...
	useEffect( () => {
		...
		if (
			...
			JSON.stringify( lastSerialised.current )
		) {
	...
	const removeRule = ( gi, ri ) => {
		const next = groups.map( ( g, i ) =>
			i === gi ? { rules: g.rules.filter( ( _, j ) => j !== ri ) } : g
		);
		update( next );
	};
		...
		<div style={ { ...S.metaStrip, marginTop: 20 } }>
			...
			>
				...
				<span style={ { ...S.muted, fontSize: 12 } }>
					Show this field set only when the conditions below match.
					Multiple groups are OR'd.
				</span>
			</div>
					...
					{ group.rules.map( ( rule, ri ) => (
						...
						>
							...
							{ valueSuggestionsForKey( rule.key ).length > 0 && (
								<datalist
									id={ `gcb-rule-values-${ rule.key }` }
								>
									{ valueSuggestionsForKey( rule.key ).map(
										( v ) => (
...
function TaxonomyTermValueInput( { value, onChange } ) {
	const v =
		typeof value === 'object' && value ? value : { taxonomy: '', term: '' };
	return (
		<div style={ { display: 'flex', gap: 8 } }>
			<input
				...
				value={ v.taxonomy || '' }
				onChange={ ( e ) =>
					onChange( { ...v, taxonomy: e.target.value } )
				}
...
function fetchIconsOnce() {
	if ( _iconCache ) {
		return Promise.resolve( _iconCache );
	}
	if ( _iconPromise ) {
		return _iconPromise;
	}
	...
	return (
		<div style={ { position: 'relative', display: 'inline-block' } }>
			<button
				...
				aria-label={
					currentIcon
						? `Icon: ${ currentIcon.label }. Click to change.`
						: 'Pick an icon'
				}
				style={ {
					minWidth: 42,
				...
				{ currentIcon ? (
					<span
						...
						dangerouslySetInnerHTML={ {
							__html: scaleSvg( currentIcon.content, 20 ),
						} }
					/>
				) : (
					<span aria-hidden style={ { color: T.ink3, fontSize: 18 } }>
						◇
					</span>
				) }
			</button>
			<span
				...
				style={ {
					...
					justifyContent: 'center',
					fontSize: 11,
					boxShadow: '0 1px 3px rgba(17,17,20,0.15)',
					pointerEvents: 'none',
				} }
			>
				✎
			...
			{ open && (
				<div
					...
					style={ {
						...
						border: `1px solid ${ T.border }`,
						borderRadius: 10,
						boxShadow: '0 8px 24px rgba(17,17,20,0.10)',
						padding: 12,
						zIndex: 100,
						width: 320,
					} }
		...
		>
			{ icon ? (
				<span
					...
					dangerouslySetInnerHTML={ {
						__html: scaleSvg( icon.content, size ),
					} }
				/>
			) : (
...
const FIELD_CATEGORIES = [
	...
	{
		label: 'Content',
		types: [ 'heading', 'heading-level', 'richtext', 'wysiwyg', 'message' ],
	},
...
function NewFieldInput( { types, onAdd } ) {
	...
	const grouped = useMemo( () => {
		...
		for ( const cat of FIELD_CATEGORIES ) {
			const items = cat.types.filter(
				( t ) => inSet.has( t ) && matchSet.has( t )
			);
			items.forEach( ( t ) => seen.add( t ) );
			if ( items.length > 0 ) {
				out.push( { label: cat.label, items } );
			}
		}
		const other = matches.filter( ( t ) => ! seen.has( t ) );
		if ( other.length > 0 ) {
			out.push( { label: 'Other', items: other } );
		}
		return out;
	}, [ query, types ] );
	...
	return (
		<div ref={ wrapRef } style={ { position: 'relative', marginTop: 8 } }>
			{ ! open ? (
				...
				>
					<span>+ Add field</span>
					<span aria-hidden style={ { color: T.ink3, fontSize: 10 } }>
						▾
					</span>
				</button>
			) : (
...
const PropertyEditor = forwardRef( function PropertyEditor(
	{ field, siblingFields, errors, onChange },
	ref
) {
	...
	useEffect( () => {
		let cancelled = false;
		getControlDocs( field.type ).then( ( d ) => {
			if ( ! cancelled ) {
				setDocs( d );
			}
		} );
	...
	const validProps = useMemo( () => {
		const fromDocs = Array.isArray( docs?.configOptions )
			? docs.configOptions
			: [];
		const base = [
			...UNIVERSAL_PROPS,
	...
	const attrKeys = useMemo(
		() =>
			siblingFields
				.map(
					( f ) =>
						f.props.find( ( [ k ] ) => k === 'attributeKey' )?.[ 1 ]
				)
				.filter( Boolean ),
		[ siblingFields ]
	);
	...
	const setProp = ( idx, [ k, v ] ) => {
		onChange( {
			...field,
			props: field.props.map( ( p, i ) => ( i === idx ? [ k, v ] : p ) ),
		} );
	};
	...
	const removeProp = ( idx ) => {
		onChange( {
			...field,
			props: field.props.filter( ( _, i ) => i !== idx ),
		} );
	};
				...
				{ field.props.map( ( [ k, v ], idx ) => {
					...
					return (
						<PropRow
							...
							k={ k }
							v={ v }
							propSpec={ validProps.find(
								( p ) => p.name === k
							) }
							validProps={ validProps.filter(
								( p ) =>
									p.name !== k &&
									! field.props.some(
										( [ pk ] ) => pk === p.name
...
const PropRow = forwardRef( function PropRow(
	...
	const isTokenKeysRow =
		k === 'tokenKeys' && TOKEN_VALUE_TYPES.has( fieldType );
	...
	return (
		<div
			style={ {
				...S.propRow,
				alignItems: isExpandedRow ? 'flex-start' : 'center',
				padding: isExpandedRow ? '6px 0' : '2px 0',
			} }
		>
			{ mandatory ? (
				<span
					...
					title="Required — can't be removed"
				>
					{ k }
				</span>
			) : (
			...
			{ isOptionsRow ? (
				<div
					style={ { width: '100%', ...errorWrap( error ) } }
				...
				<AutoInput
					...
					style={ {
						...S.propValue,
						...( error
							? { borderColor: '#d63638', background: '#fdf2f2' }
							: null ),
					} }
					title={ error || undefined }
					onChange={ ( nv ) =>
						onChange( [ k, coerceValue( nv, propSpec ) ] )
					}
					onCommit={ () => onNext?.() }
					onTab={ onNext }
				/>
			...
			{ mandatory ? (
				<span style={ S.propRemove } title="Required" aria-hidden>
					·
				</span>
			) : (
				<button
...
function TokenKeysEditor( {
	...
	useEffect( () => {
		...
		loadTokenTree()
			...
			.catch( () => {
				if ( live ) {
					setErr( 'Could not load theme tokens.' );
				}
			} );
		return () => {
			live = false;
	...
	const groups = useMemo( () => {
		...
		const filtered = all.filter( ( g ) =>
			allowedCats.includes( g.id.split( ':' )[ 0 ] )
		);
	...
	const activeGroupId =
		group && groups.some( ( g ) => g.id === group )
			? group
			: groups.find( ( g ) => g.id === DEFAULT_TOKEN_GROUP[ fieldType ] )
					?.id ||
			  groups[ 0 ]?.id ||
			  '';
	...
	useEffect( () => {
		if ( ! group && activeGroupId && groups.length ) {
			onChange( activeGroupId, keys );
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [ groups.length ] );

	if ( err ) {
		return <div className="gcb-tok-state">{ err }</div>;
	}
	if ( ! tree ) {
		return <div className="gcb-tok-intro">Loading theme tokens…</div>;
	}
	if ( ! groups.length ) {
		return (
			<div className="gcb-tok-intro">
				This theme has no design tokens in theme.json yet.
			</div>
		);
	}
	...
	const all = ( on ) =>
		onChange(
			activeGroupId,
			on
				? ( activeGroup?.tokens || [] ).map( ( t ) => t.slug || t.key )
				: []
		);

	const setDefault = ( val ) =>
		setOne( 'default', defaultVal === val ? '' : val );
	...
	const defRadio = ( val ) => (
		<input
			type="radio"
			className="gcb-tok-default"
			checked={ defaultVal === val }
			onClick={ () => setDefault( val ) }
			onChange={ () => {} }
	...
	return (
		<div>
			<div className="gcb-tok-intro">
				Which of your theme tokens should this field offer? (All by
				default — uncheck to narrow.)
			</div>
			{ groups.length > 1 && (
				<select
					className="gcb-tok-group"
					value={ activeGroupId }
					onChange={ ( e ) => onChange( e.target.value, [] ) }
				>
					{ groups.map( ( g ) => (
						<option key={ g.id } value={ g.id }>
							{ g.label } ({ g.tokens.length })
						</option>
					) ) }
				</select>
			) }
							...
							return (
								<div key={ slug } className="gcb-tok-row">
									<input
										...
										title="Offer this token"
									/>
									<span className="gcb-tok-label">
										{ t.label || t.key }
									</span>
									<code className="gcb-tok-slug">
										{ slug }
									</code>
									{ t.swatch ? (
										<span
											className="gcb-tok-swatch"
						...
						<div className="gcb-tok-custom">
							...
							{ customList.map( ( c ) => (
								<div key={ c } className="gcb-tok-row">
									<code className="gcb-tok-slug gcb-tok-label">
										{ c }
									</code>
									<span
										className="gcb-tok-swatch"
...
function TokenPicker( { value, onChange } ) {
	...
	const checkAll = ( on ) => {
		...
		if ( on ) {
			...
			const add = group.tokens
				.map( tokenToOption )
				.filter( ( o ) => ! have.has( o.value ) );
			onChange( [ ...opts, ...add ] );
		} else {
			const groupVals = new Set(
				group.tokens.map( ( t ) => tokenToOption( t ).value )
			);
	...
	const removeOpt = ( val ) =>
		onChange( opts.filter( ( o ) => o.value !== val ) );

	// --- custom token add ---
	const startAdd = () => setAdding( { slug: '', val: '' } );
	const commitOneOff = () => {
		if ( ! adding?.slug.trim() || ! adding?.val.trim() ) {
			return;
		}
	...
	const commitToTheme = async () => {
		if ( ! adding?.slug.trim() || ! adding?.val.trim() || ! group ) {
			return;
		}
		try {
			const res = await apiFetch( {
				...
				data: {
					category: group.type,
					slug: adding.slug.trim(),
					value: adding.val.trim(),
				},
			} );
			...
			onChange( [
				...opts,
				{
					label: t?.label || adding.slug.trim(),
					value: t?.key || adding.slug.trim(),
				},
			] );
			loadTokenTree()
				.then( setTree )
			...
			setErr(
				( e?.message || 'Could not write to theme.json' ) +
					' — added as a one-off instead.'
			);
			commitOneOff();
		}
	};
	...
	if ( err && ! tree ) {
		return (
			<div style={ { ...S.muted, fontSize: 12, color: T.danger } }>
				{ err }
			</div>
		);
	}
		...
		<div style={ { width: '100%' } }>
			...
			{ opts.some( ( o ) => o._custom ) && (
				<div style={ { marginTop: 8 } }>
					<div
						style={ { ...S.muted, fontSize: 11, marginBottom: 2 } }
					>
						Custom (this field only)
					</div>
					{ opts
						.filter( ( o ) => o._custom )
						.map( ( o ) => (
							<div
								key={ o.value }
								style={ { ...S.tokRow, cursor: 'default' } }
							>
								<span style={ { flex: 1 } }>{ o.label }</span>
								<code style={ S.tokSlug }>{ o.value }</code>
								<button
									type="button"
									onClick={ () => removeOpt( o.value ) }
									style={ S.propRemove }
									title="Remove"
								>
									✕
				...
				>
					...
					>
						<input
							type="text"
							value={ adding.slug }
							placeholder="name (e.g. brand-pink)"
							onChange={ ( e ) =>
								setAdding( { ...adding, slug: e.target.value } )
							}
							style={ {
								...S.input,
								padding: '5px 7px',
						...
						<input
							...
							value={ adding.val }
							placeholder={
								group?.type === 'color' ? '#ff3399' : 'value'
							}
							onChange={ ( e ) =>
								setAdding( { ...adding, val: e.target.value } )
							}
							style={ {
								...S.input,
								padding: '5px 7px',
					...
					>
						Not in your theme tokens. Add it just here, or save it
						to your theme so every block can use it?
					</div>
					<div style={ { display: 'flex', gap: 6 } }>
						<button
							type="button"
							onClick={ commitOneOff }
							style={ S.tokBtn }
						>
							Add one-off
						</button>
						<button
							type="button"
							onClick={ commitToTheme }
							style={ { ...S.tokBtn, ...S.tokBtnPrimary } }
						>
							Add to my theme
						</button>
						<button
...
function OptionsEditor( { value, onChange } ) {
	...
	const [ useTokens, setUseTokens ] = useState( () =>
		opts.some( ( o ) => o._fromToken )
	);
...
function RepeaterFieldsEditor( { value, onChange } ) {
	...
	return (
		<div style={ { width: '100%' } }>
			{ rows.length === 0 && (
				<div style={ { ...S.muted, fontSize: 12, padding: '4px 0' } }>
			...
			{ rows.map( ( r, i ) => (
				...
				>
					...
					<input
						...
						onChange={ ( e ) =>
							update( i, 'attributeKey', e.target.value )
						}
						style={ {
							...S.input,
							padding: '4px 6px',
...
const AddPropRow = forwardRef( function AddPropRow(
	{ validProps, onAdd },
	ref
) {
...
function fieldToControl( field ) {
	...
	for ( const [ k, v ] of field.props ) {
		if ( k === 'type' ) {
			continue;
		} // type is held on the field directly
...
function suggestValues( propSpec, key, fieldType, ctx ) {
	...
	if ( key === 'parentPanelId' ) {
		return ctx.panelIds;
	}
	if ( key === 'conditionalLogic.field' ) {
		return ctx.attrKeys;
	}
	if ( key === 'type' ) {
		return CONFIG.controlTypes || [];
	}
	...
	if ( key === 'category' ) {
		return [ 'widgets', 'text', 'media', 'design', 'theme', 'embed' ];
	}
	if ( key === 'icon' ) {
		return [
			'core/layout',
	...
	if ( key === 'variant' && fieldType === 'message' ) {
		return [ 'neutral', 'info', 'warning', 'danger', 'success' ];
	}
	if ( key === 'conditionalLogic.operator' ) {
		return [
			'=',
			'!=',
			'>',
...
function jsType( label ) {
	...
	if ( n.startsWith( 'boolean' ) ) {
		return 'boolean';
	}
	if ( n.startsWith( 'number' ) || n.startsWith( 'integer' ) ) {
		return 'number';
	}
...
function validateField( field, allFields ) {
	...
	if ( ! id || String( id ).trim() === '' ) {
		...
		const dupId = allFields
			.filter( ( f ) => f !== field )
			.some(
				( f ) => f.props.find( ( [ k ] ) => k === 'id' )?.[ 1 ] === id
			);
		if ( dupId ) {
	...
	if ( ! isStructural( type ) ) {
		if ( ! attrKey || String( attrKey ).trim() === '' ) {
			...
			errors.attributeKey =
				'Use letters, digits, underscores; start with a letter or underscore';
		} else {
			const dupKey = allFields
				.filter( ( f ) => f !== field )
				.some(
					( f ) =>
						f.props.find(
			...
			if ( dupKey ) {
				errors.attributeKey = 'Duplicate attributeKey';
			}
		}
	}
	...
	if ( parent ) {
		const panels = allFields
			.filter( ( f ) => f !== field && isStructural( f.type ) )
			.map( ( f ) => f.props.find( ( [ k ] ) => k === 'id' )?.[ 1 ] );
	...
	if ( condField ) {
		...
		if ( ! keys.includes( condField ) ) {
			errors[ 'conditionalLogic.field' ] =
				'No sibling with that attributeKey';
		}
	}
...
function FormRow( { label, help, children } ) {
	return (
		<div style={ { marginBottom: 16 } }>
			...
			{ children }
			{ help && (
				<div style={ { fontSize: 12, color: '#525260', marginTop: 4 } }>
					{ help }
				</div>
			) }
		</div>
...
const S = {
	...
	propRow: {
		display: 'grid',
		gridTemplateColumns: '200px 12px 1fr 24px',
		alignItems: 'center',
		gap: 4,
		padding: '6px 0',
		borderBottom: `1px solid ${ T.border }`,
	...
	suggestList: {
		...
		background: T.surface,
		border: `1px solid ${ T.border }`,
		boxShadow: '0 6px 18px rgba(17, 17, 20, 0.08)',
		zIndex: 10,
		maxHeight: 240,
		overflowY: 'auto',
		borderRadius: 8,
	...
	metaGrid: {
		display: 'grid',
		gridTemplateColumns:
			'minmax(220px, 1fr) minmax(160px, auto) minmax(180px, auto)',
		gap: 16,
		alignItems: 'end',
	},
	metaLabel: {
