...
} from '@wordpress-gcb/fields/conditional-logic';
...
describe( 'shouldRender — operators', () => {
	test( '==', () => {
		expect(
			shouldRender(
				cond( [ { field: 'f', operator: '==', value: true } ] ),
				{ f: true }
			)
		).toBe( true );
		expect(
...
describe( 'panelsContainingErrors', () => {
	...
	test( 'finds multiple panels when several have errors', () => {
		...
		const errored = panelsContainingErrors( controls, {
			a: 'msg',
			b: 'msg',
		} );
	...
	test( 'ignores ungrouped errored fields (no parentPanelId)', () => {
		...
		expect( panelsContainingErrors( controls, { a: 'msg' } ).size ).toBe(
			0
		);
	} );
	...
	test( 'ignores structural controls', () => {
		...
		expect( panelsContainingErrors( controls, { g1: 'msg' } ).size ).toBe(
			0
		);
	} );
} );
