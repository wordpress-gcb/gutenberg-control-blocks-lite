...
describe( 'validate — required', () => {
	...
	test( 'required populated string ok', () => {
		expect(
			validate( ctrl( { validation: { required: true } } ), 'hello' ).ok
		).toBe( true );
	} );

	test( 'zero is not empty (toggle/number = 0 is a valid value)', () => {
		expect(
			validate( ctrl( { validation: { required: true } } ), 0 ).ok
		).toBe( true );
		expect(
			validate( ctrl( { validation: { required: true } } ), '0' ).ok
		).toBe( true );
	...
	test( 'custom requiredMessage string', () => {
		const r = validate(
			ctrl( {
				validation: {
					required: true,
					requiredMessage: 'Custom required text.',
				},
			} ),
			''
		);
	...
	test( 'required as object with message', () => {
		const r = validate(
			ctrl( {
				validation: { required: { message: 'Object form message.' } },
			} ),
			''
		);
...
describe( 'validate — length', () => {
	...
	test( 'minLength pass', () => {
		expect(
			validate( ctrl( { validation: { minLength: 3 } } ), 'abcd' ).ok
		).toBe( true );
	} );

	test( 'maxLength fail', () => {
		expect(
			validate( ctrl( { validation: { maxLength: 3 } } ), 'abcde' ).ok
		).toBe( false );
	} );

	test( 'empty optional skips length check', () => {
		expect(
			validate( ctrl( { validation: { minLength: 5 } } ), '' ).ok
		).toBe( true );
	} );
} );
...
describe( 'validate — pattern', () => {
	...
	test( 'pattern pass', () => {
		expect(
			validate( ctrl( { validation: { pattern: '^[A-Z]' } } ), 'Capital' )
				.ok
		).toBe( true );
	} );
	...
	test( 'invalid regex is treated as no constraint, not fatal', () => {
		const spy = jest
			...
			.mockImplementation( () => {} );
		const r = validate(
			ctrl( { validation: { pattern: '[unclosed' } } ),
			'anything'
		);
...
describe( 'validateAll', () => {
	...
	test( 'respects isVisible — hidden required fields are skipped', () => {
		...
		expect( validateAll( controls, {}, () => false ) ).toEqual( {
			ok: true,
		} );
	} );
